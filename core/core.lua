...
local panelFrame = CreateFrame("Frame", "CyberDeckPanel", UIParent, BackdropTemplateMixin and "BackdropTemplate")
...
-- 显示/隐藏按钮
local controlButton = CreateFrame("Button", "CyberDeckControlButton", UIParent, "UIPanelButtonTemplate")
...
local bindEditBox = CreateFrame("EditBox", "CyberDeckBindEditBox", UIParent)
bindEditBox:Hide()
...
bindEditBox:SetScript("OnKeyDown", function(self, key)
    ...
    if key == "LSHIFT" or key == "RSHIFT" or key == "LCTRL" or key == "RCTRL"
        ...
        print("|cFFFFD700CyberDeck|r: 不支持修饰键，请只按普通键")
        return
    end
    if IsShiftKeyDown() or IsControlKeyDown() or IsAltKeyDown() then
        print("|cFFFFD700CyberDeck|r: 不支持修饰键组合，请只按普通键")
        return
    end
    ...
    if key == "ESCAPE" then
        ...
        print("|cFFFFD700CyberDeck|r: 快捷键已清除")
        return
    end
    ...
    print("|cFFFFD700CyberDeck|r: " .. key .. " 已绑定")
end)
...
local function createSwitchButton(opt)
    ...
    btn:SetScript("OnClick", function(self, mouseBtn)
        if mouseBtn == "RightButton" then
            ...
            print("|cFFFFD700CyberDeck|r: 按下要绑定的按键 (ESC取消), 不支持修饰键和组合")
            bindEditBox:Show()
