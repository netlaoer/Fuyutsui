...
2. 安装 VS Code
安装 VS Code（任意版本即可）。
打开 VS Code 后：文件 -> 打开文件夹，选择项目根目录, 也就是Interface/AddOns/Fuyutsui。
3. 安装 Python（Windows）
桌面端脚本依赖 Python 3，且会调用 Windows API，请在 Windows 上安装。

3.1 从哪里装
打开 Python 官方下载页，下载 Windows installer (64-bit)。
版本建议 3.10 或更高（与 customtkinter、类型提示等兼容性更好；3.9 多数情况也能用，若遇库不兼容再升级）。
3.2 安装向导里务必勾选的选项
...
cd "你的路径\Interface\AddOns\Fuyutsui\Fuyutsui"
若使用了虚拟环境，先执行 .\.venv\Scripts\activate。
...
若报错，把完整终端输出复制给 AI 或查阅报错中的包名，通常是网络、代理或 Python 版本过旧导致。

5. 运行与日常使用
5.1 与游戏配合
启动魔兽世界，在角色选择界面或游戏中于插件列表启用 Fuyutsui。
桌面端通过窗口标题查找游戏窗口，默认标题为 魔兽世界（与国服客户端一致；若你使用其他语言客户端，需改 GetPixels.py / utils.py 里传入的 window_title 或相关默认值，否则截不到正确区域）。
5.2 启动桌面 GUI
...
免责声明
本项目偏“个人工具/实验性质”，通过读取游戏画面像素并触发热键来实现辅助决策。 请你自行判断是否符合你的需求，别让它被荒坂发现。
