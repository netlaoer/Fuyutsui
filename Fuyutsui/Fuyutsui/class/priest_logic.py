...
def _priest_holy_logic(state_dict):
    ...
    if 引导 > 0:
        ...
        current_step = "施放 群体驱散"
        action_hotkey = get_hotkey(0, "群体驱散")
    elif 神圣赞美诗_cd == 0 and state_dict.get("法术失败") == 31:
        current_step = "施放 神圣赞美诗"
        action_hotkey = get_hotkey(0, "神圣赞美诗")
    elif 光晕_cd == 0 and state_dict.get("法术失败") == 30:
        current_step = "施放 光晕"
        action_hotkey = get_hotkey(0, "光晕")
    elif 神圣化身_cd == 0 and state_dict.get("法术失败") == 29:
        current_step = "施放 神圣化身"
        action_hotkey = get_hotkey(0, "神圣化身")
    elif 英雄天赋 == 3:
        if 队伍类型 <= 40:
            if 纯净术_cd == 0 and dispel_unit is not None and (首领战 == 4 or 首领战 == 5):
                current_step = f"施放 纯净术 on {dispel_unit}"
                action_hotkey = get_hotkey(int(dispel_unit), "纯净术")
            # 愈合祷言
            elif 愈合祷言_cd == 0 and no_mend_u is not None and no_mend_p is not None and no_mend_p < 95:
                ...
                if 静_charge <= 3 and lowest_p < 80:
                    ...
                    action_hotkey = get_hotkey(int(lowest_u), "圣言术：静")
                # 神圣化身
                elif 神圣化身_cd == 0 and count80 >= 4 and 静_cd > 5:
                    current_step = "施放 神圣化身"
                    action_hotkey = get_hotkey(0, "神圣化身")
                # 光晕
                elif 施法技能 != 17 and count90 >= 4 and 光晕_cd == 0:
                    current_step = "施放 光晕"
                    action_hotkey = get_hotkey(0, "光晕")
                # 治疗祷言
                ...
                elif 织光层数 > 0 and not 移动 and 施法技能 != 11 and count90 >= 3:
                    current_step = "施放 治疗祷言"
                    action_hotkey = get_hotkey(0, "治疗祷言")
                elif 织光层数 > 0 and not 移动 and 施法技能 == 11 and (count80 >= 3 or count90 >= 7):
                    current_step = "施放 治疗祷言"
                    action_hotkey = get_hotkey(0, "治疗祷言")
                # 快速治疗
                elif 织光层数 < 4:
            ...
            # 纯净术
            if 纯净术_cd == 0 and dispel_unit is not None:
                current_step = f"施放 纯净术 on {dispel_unit}"
                action_hotkey = get_hotkey(int(dispel_unit), "纯净术")
            ...
            if lowest_u is not None and lowest_p is not None and lowest_p < 90:
                # 圣言术：静
                if 静_charge <= 3 and lowest_p < 80:
                    current_step = f"施放 圣言术：静 on {lowest_u}, 生命低于80%的单位"
                    action_hotkey = get_hotkey(int(lowest_u), "圣言术：静")
                    ...
                    action_hotkey = get_hotkey(int(lowest_u), "圣言术：静")
                # 神圣化身
                elif 战斗 and 神圣化身_cd == 0 and count80 >= 3 and 静_cd > 5:
                    current_step = "施放 神圣化身"
                    action_hotkey = get_hotkey(0, "神圣化身")
                # 光晕
...
def _priest_shadow_logic(state_dict):
    ...
    elif 目标有效 and spells.get("虚空形态") < 1 and state_dict.get("法术失败") == 34:
